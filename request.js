...
const Fetch =(opt)=> {
    if(typeof opt === 'string') {
        return fetch(opt).then((res)=>res.json())
    }else {
...
Fetch.GET = (opt) => {
    ...
    if( params !== null && Object.keys(params).length ) {
        ...
        return fetch(url).then(res=> res.json() || res.arrayBuffer())
    }
}
...
Fetch.POST = (opt) => {
    ...
    return fetch(url, newOpt).then(res=>res.json() || res.arrayBuffer())
}
...
Fetch.DELETE = (opt) => {
    ...
    return fetch(url, newOpt).then(res=>res.json())
}
